...
export default function DashboardVen({ user, onLogout }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await fetch('http://127.0.0.1:5001/dashboard-ven', {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card title={"Mis Leads"} value={info.leads} ></Card>
            <Card title={"Mis Ventas"} value={info.ventas} ></Card>       
